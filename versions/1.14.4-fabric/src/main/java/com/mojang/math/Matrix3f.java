...
public final class Matrix3f {
    ...
    public Matrix3f(Matrix4f matrix4f) {
        float[] values = ((Matrix4fAccessor) (Object) matrix4f).magiclib$getValues();
        this.m00 = values[0];
        this.m01 = values[4];
        this.m02 = values[8];
        this.m10 = values[1];
    ...
    private static Quaternion stepJacobi(@NotNull Matrix3f matrix3f) {
        ...
        Quaternion quaternion = QuaternionCompat.ONE.get();
    ...
    public Triple<Quaternion, Vector3f, Quaternion> svdDecompose() {
        QuaternionCompatImpl quaternion = (QuaternionCompatImpl) QuaternionCompat.ONE;
        QuaternionCompatImpl quaternion2 = (QuaternionCompatImpl) QuaternionCompat.ONE;
        Matrix3f matrix3f = this.copy();
        matrix3f.transpose();
        matrix3f.mul(this);

        for (int i = 0; i < 5; ++i) {
            quaternion2.get().mul(stepJacobi(matrix3f));
        }

        quaternion2.normalize();
        Matrix3f matrix3f2 = new Matrix3f(this);
        matrix3f2.mul(new Matrix3f(quaternion2.get()));
        float f = 1.0F;
        ...
        quaternion.get().mul(quaternion3);
        ...
        quaternion.get().mul(quaternion4);
        ...
        quaternion.get().mul(quaternion5);
        ...
        return Triple.of(quaternion.get(), vector3f, quaternion2.get());
    }
